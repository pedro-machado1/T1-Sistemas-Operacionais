...
public class Sistema {
    ...
    public class GM {
        ...
        public GM(int tamMem, int tamPg) {
            ...
            this.framesLivres = new boolean[nroFrames];
            start();
        }
        ...
        public boolean aloca(int nroPalavras, int[] tabelaPaginas) {
            int livres = 0, pagina = 0;

            int nroPaginas = (int) Math.ceil((double) nroPalavras / tamPg);

            for (boolean f : framesLivres) {
                if (f) livres++;
            }

            if (livres < nroPaginas) return false;

            for (int i = 0; i < framesLivres.length && pagina < nroPaginas; i++) {
                if (framesLivres[i]) {
                    framesLivres[i] = false;
                    tabelaPaginas[pagina++] = i;
                }
            }
            return true;
        }
    ...
    public enum Opcode {
        DATA, ___,              // se memoria nesta posicao tem um dado, usa DATA, se nao usada e NULO
    ...
    public class CPU {
        ...
        public void setContext(int _pc, int[] tabelaPaginas, int tamPg) {                 // usado para setar o contexto da cpu para rodar um processo
        ...
        private int traduzEndereco(int enderecoLogico) {
        ...
        public void run() {                               // execucao da CPU supoe que o contexto da CPU, vide acima,
            ...
            while (!cpuStop) {      // ciclo de instrucoes. acaba cfe resultado da exec da instrucao, veja cada caso.
                ...
                // --------------------------------------------------------------------------------------------------
                // FASE DE FETCH
                if (legal(traduzEndereco(pc))) { // pc valido
                    ir = m[traduzEndereco(pc)];  // <<<<<<<<<<<< AQUI faz FETCH - busca posicao da memoria apontada por pc, guarda em ir
                    ...
                    switch (ir.opc) {       // conforme o opcode (código de operação) executa
                        ...
                        case LDD: // Rd <- [A]
                            if (legal(traduzEndereco(ir.p))) {
                                reg[ir.ra] = m[traduzEndereco(ir.p)].p;
                                pc++;
                            }
                            break;

                        case LDX: // RD <- [RS] // NOVA
                            if (legal(traduzEndereco(reg[ir.rb]))) {
                                reg[ir.ra] = m[traduzEndereco(reg[ir.rb])].p;
                                pc++;
                            }
                            break;
                        ...
                        case STD: // [A] ← Rs
                            if (legal(traduzEndereco(ir.p))) {
                                ...
                                m[traduzEndereco(ir.p)].p = reg[ir.ra];
                                pc++;
                                if (debug) {
                                    System.out.print("                                  ");
                                    u.dump(traduzEndereco(ir.p), traduzEndereco(ir.p) + 1);
                                }
                            }
                            break;

                        case STX: // [Rd] ←Rs
                            if (legal(traduzEndereco(reg[ir.ra]))) {
                                m[traduzEndereco(reg[ir.ra])].opc = Opcode.DATA;
                                m[traduzEndereco(reg[ir.ra])].p = reg[ir.rb];
                                pc++;
                            }
                            break;
